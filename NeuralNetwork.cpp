...
double NeuralNetwork::Train(const vector<vector<double>>& trainingInput, const vector<double>& targetOutput, int epochs, double learningRate)
{
	double totalError = 0;
	...
	for (int e = 0; e < epochs; e++)
	{
		...
		for (size_t i = 0; i < trainingInput.size(); i++)
		{
			...
			hiddenLayer[0] = MathFunctions::Sigmoid(trainingInput[i][0] * weightsInputToHidden[0] + trainingInput[i][1] * weightsInputToHidden[1] + biasHidden[0]);
			hiddenLayer[1] = MathFunctions::Sigmoid(trainingInput[i][0] * weightsInputToHidden[2] + trainingInput[i][1] * weightsInputToHidden[3] + biasHidden[1]);

			// Calculate output value.
			double output = MathFunctions::Sigmoid(hiddenLayer[0] * weightsHiddenToOutput[0] + hiddenLayer[1] * weightsHiddenToOutput[1] + biasOutput);
...
double NeuralNetwork::Predict(const vector<double>& input)
{
	...
	hiddenLayer[0] = MathFunctions::Sigmoid(input[0] * weightsInputToHidden[0] + input[1] * weightsInputToHidden[1] + biasHidden[0]);
	hiddenLayer[1] = MathFunctions::Sigmoid(input[0] * weightsInputToHidden[2] + input[1] * weightsInputToHidden[3] + biasHidden[1]);
